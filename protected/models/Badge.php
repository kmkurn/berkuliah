...
class Badge extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, picture, description', 'required'),
			array('name', 'length', 'max'=>30),
			array('picture', 'length', 'max'=>32),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, picture, description', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'students' => array(self::MANY_MANY, 'Student', 'achievement(badge_id, student_id)'),
		);
	}
